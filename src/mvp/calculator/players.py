...
class TucanPlayer:
    ...
    def add_match_points(self, marks : List[int], discipline : TucanDiscipline, position : str) -> None:
        ...
        if not eval_params:
            raise TucanException(f"Problems retrieving evaluation parameters for '{discipline.name}' in position '{position}'.")

        # Do a quick check to see if the sizes of eval_params and marks are the same
        if len(eval_params) != len(marks):
            raise TucanException(f"Evaluation parameters for '{discipline.name}' in position '{position}' do not match the marks given.")
